...
import { requireGroupAccessFromBody, requireGroupAccessFromQuery } from "../middlewares/authMiddleware";

export const contributionsRoutes = new Hono<{ Bindings: Env; Variables: { authUserId: string } }>();

contributionsRoutes.use("*", requireAuth);
contributionsRoutes.get("/plans", requireGroupAccessFromQuery("groupId"), listContributionPlansController);
contributionsRoutes.post("/plans", requireGroupAccessFromBody("groupId"), createContributionPlanController);
